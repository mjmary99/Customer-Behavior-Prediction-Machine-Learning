...
## MUTATORS ####################################################################
    ...
    def setActivation(self, method, sig=None, d_sig=None, sig0=None, d_sig_0=None ): 
        """
        ...
        if method == 'logistic':
            self.sig = lambda z: z if z > 0 else 0
            self.d_sig = lambda z: 1 if z > 0 else 0
            self.sig_0 = lambda z: twod(1 / (1 + np.exp(-z)))
            self.d_sig_0 = lambda z: twod(np.multiply(self.sig(z), (1 - self.sig(z))))
